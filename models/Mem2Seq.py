...
class Dataset(torch.utils.data.Dataset):
    ...
    def __init__(self,sourceSeq,targetSeq,trg_plain,indexSeq, gateSeq,source_word2id, target_word2id,max_len,entity,entity_cal,entity_nav,entity_wet, conv_seq, KB_arr):
        super(MyDataset, self).__init__()
        self.sourceSeq = sourceSeq
        self.src_plain = sourceSeq
        self.targetSeq = targetSeq
        self.trg_plain = trg_plain
        self.indexSeq = indexSeq
        self.gateSeq = gateSeq
        self.totalSeqs = len(self.sourceSeq)
    ...
    def __getitem__(self, index):
        ...
        src_seq = self.sourceSeq[index]
        trg_seq = self.targetSeq[index]
        index_s = self.indexSeq[index]
        trg_plain  = self.trg_plain[index]
        src_plain  = self.src_plain[index]
        src_seq = self.preprocess(src_seq, self.source_word2id, trg=False)
        index_s = self.preprocess_inde(index_s,src_seq)
        # gete_s  = self.preprocess_gate(gete_s)
        
        return src_seq, trg_seq, index_s, trg_plain,self.maxLen, src_plain

    def __len__(self):
        return self.totalSeqs

    def preprocess(self,sequence,word2id,target = True) -> None:
        """Converts words to ids"""
        if target:
            story = [word2id[word] if word in word2id else ]
...
class EncoderMemNN(nn.Module):
    def __init__(self,vocab,embedding_dim,hops,dropout,unk_mask):
        ...
        for hop in range(self.max_hops+1):
            embeddedUnit = nn.Embedding(self.num_vocab,embedding_dim,padding_idx=PAD_token)
    ...
    def get_state(self,batchsize):
        if USE_CUDA:
            ...
            return Variable(torch.zeros(batchsize,self.embedding_dim))
    ...
    def forward(self,story):
        ...
        if self.unk_mask:
            if self.training:
                ones = np.ones((story[0],story[1],story[2]))
                rand_mask = np.random.binomial([np.ones((story[0],story[1]))],1-self.dropout)[0]
                ones[:,:,0] = ones[:,:,0]*rand_mask
                cur_tensor = Variable(torch.Tensor(ones).to('cuda'))
                if USE_CUDA:
        ...
        for hop in range(self.max_hops):
            ...
            embed_A = self.embeddedUnit[hop](story.contiguous().view(story.size(0),-1).long()) # b * (m * s) * e
            embed_A = embed_A.view(story_size+(embed_A.size(-1))) # b * m * s * e
            m_A = torch.sum(embed_A,2).squeeze(2) #b* m * e  C_i^k

            u_temp = u[-1].unsqueeze(1).expand_as(m_A) #(q^k)^T
            prob = self.softmax(torch.sum(u_temp*embed_A,2)) #p_i^k
            embed_C = self.embeddedUnit[hop+1](story.contiguous().view(story.size(0),-1).long())
...
class DecoderMemNN(nn.Module):
    ...
    def ptrMemDecoder(self, enc_query, last_hidden):
        ...
        for hop in range(self.max_hops):
            ...
            if(len(list(u[-1].size()))==1): u[-1] = u[-1].unsqueeze(0) ## used for bathcsize = 1.
            ...
            if (hop==0):
                p_vocab = self.W1(torch.cat((u[0], o_k),1)) # vocabulary distribution
...
class Mem2Seq(nn.Module):
    def __init__(self,hiddenSize,max_len, max_r,path,lang,task,learningRate,n_layers,dropout,unk_mask):
    ...
    def evaluate_batch(self,batch_size,input_batches, input_lengths, target_batches, target_lengths, target_index,target_gate,src_plain):  
        ...
        # Run words through encoder
        decoder_hidden = self.encoder(input_batches).unsqueeze(0) # encoder forward result
        self.decoder.load_memory(input_batches.transpose(0,1))
        ...
        for t in range(self.max_r):
            decoder_ptr,decoder_vacab, decoder_hidden = self.decoder.ptrMemDecoder(decoder_input, decoder_hidden)
            all_decoder_outputs_vocab[t] = decoder_vacab
            topv, topvi = decoder_vacab.data.topk(1) # top_vocabulary,top_vacab_index
            all_decoder_outputs_ptr[t] = decoder_ptr
            topp, toppi = decoder_ptr.data.topk(1) # top_ptr,top_ptr_index
            top_ptr_i = torch.gather(input_batches[:,:,0], 0, Variable(toppi.view(1, -1))).transpose(0,1)
            next_in = [top_ptr_i[i].item() if (toppi[i].item() < input_lengths[i]-1) else topvi[i].item() for i in range(batch_size)]

            decoder_input = Variable(torch.LongTensor(next_in)) # Chosen word is next input，将选好的词作为下一个词的输入，回瞥
            if USE_CUDA: decoder_input = decoder_input.cuda()
        ...
        for j, data_dev in pbar: 
            ...
            for i, row in enumerate(np.transpose(words)):
                ...
                if args['dataset']=='babi':
                    ...
                    if (correct == st): #成功输出正确的句子
                        acc+=1
                        dialog_acc_dict[data_dev[11][i]].append(1) #对话矩阵one-hot编码，1代表正确回答的选项
                    else:
